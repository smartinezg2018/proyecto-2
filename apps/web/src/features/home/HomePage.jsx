import { Package, Shield, Receipt, BarChart3, AlertTriangle } from 'lucide-react';
import {
  BarChart,
  Bar,
  XAxis,
...
export function ModuleHome() {
  ...
  const alDia = APARTMENTS.filter((a) => a.status === 'Al d�a').length;
  const expiringSoon = POLICIES.filter((p) => p.daysLeft < 30);

  const pieData = [
    { name: 'Al d�a', value: alDia },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          sub={`${moraCount} aptos en mora`}
          icon={Receipt}
          color="red"
        />
        <MetricCard
          label="Ejecuci�n Presupuestal"
          value="58%"
            ...
            {APARTMENTS.filter((a) => a.daysLate > 60).map((a) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-400">Cartera cr�tica � {fmt(a.balance)}</p>
                </div>
                <span className="text-xs font-bold text-amber-700 bg-amber-100 px-2.5 py-1 rounded-full">
                  {a.daysLate} d�as mora
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200/80 p-5">
          <h3 className="text-sm font-bold text-slate-700 mb-4">
            Recaudo vs Mora � �ltimos 6 meses
          </h3>
