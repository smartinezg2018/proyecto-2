...
import { BarChart3, Plus, AlertTriangle, TrendingUp, FileText } from 'lucide-react';
import {
  BarChart,
  Bar,
  XAxis,
...
export function ModuleFacturacion() {
  ...
  const recaudoPct = Math.round(
    (APARTMENTS.filter((a) => a.status === 'Al d�a').length / APARTMENTS.length) * 100
  );

  return (
    <div>
      <SectionHead
        title="Facturaci�n y Recaudo"
        subtitle="Control de cartera, facturaci�n y pagos del conjunto"
        action={
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <MetricCard
          label="% de Recaudo"
          value={`${recaudoPct}%`}
          sub="Sobre cuota ordinaria mensual"
          icon={BarChart3}
          color="blue"
        />
      </div>
      ...
      <div className="flex gap-0 mb-4 bg-slate-100 rounded-lg p-1 w-fit">
        {[
          { key: 'cartera', label: 'Estado de Cartera' },
          { key: 'historial', label: 'Historial de Recaudo' }
        ].map((t) => (
          <button
            key={t.key}
      ...
      {tab === 'cartera' ? (
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {APARTMENTS.map((a) => (
                <tr
                  key={a.unit}
                  className={`border-b border-slate-50 hover:bg-slate-50 transition-colors ${a.daysLate > 60 ? 'bg-red-50/30' : ''}`}
                >
                  <td
                  ...
                  >
                    {a.balance > 0 ? (
                      <span className="text-red-600">{fmt(a.balance)}</span>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    <button className="text-xs text-blue-600 hover:text-blue-800 font-semibold hover:underline">
                      Ver detalle
                    </button>
                  </td>
                </tr>
            ...
            <div className="flex items-center gap-4 text-xs text-slate-500">
              ...
              <span>
                ...
                <strong className="text-emerald-600">
                  {APARTMENTS.filter((a) => a.status === 'Al d�a').length}
                </strong>
              </span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-5">
          <h3 className="text-sm font-bold text-slate-700 mb-4">
            Recaudo mensual vs Mora � �ltimos 6 meses
          </h3>
