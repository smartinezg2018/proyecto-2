...
export function StatusBadge({ label }) {
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${statusClasses[label] ?? 'bg-slate-100 text-slate-500 ring-1 ring-slate-200'}`}
    >
      {label}
    </span>
  );
