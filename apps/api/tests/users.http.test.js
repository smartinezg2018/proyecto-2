...
test('HTTP: registro 201, validación 400, duplicados 409 y errores 500', async (t) => {
  const users = [];
  t.mock.method(UserRepository.prototype, 'findByEmail', async (email) => users.find((user) => user.email === email));
  t.mock.method(UserRepository.prototype, 'findByIdentification', async (id) => users.find((user) => user.identification === id));
  ...
  try {
    ...
    const input = { identification: '123', name: 'Juan', email: 'juan@email.com', status: 'active' };
    const post = (body) => fetch(`${baseUrl}/api/v1/administration/users`, {
    ...
    create.mock.mockImplementation(async () => { throw new Error('Internal database details'); });
