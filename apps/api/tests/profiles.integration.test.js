...
test(
  'persiste perfiles y asociaciones en MySQL y rechaza permisos inexistentes',
  {
    skip:
  ...
  async () => {
    ...
    try {
      const migration = await fs.readFile(
        new URL('../database/migrations/002_profiles.sql', import.meta.url),
        'utf8'
      );
      ...
      for (const permissionIds of [[], [permissionId, permissionId]]) {
        const profile = await useCases.create(
          { name: 'Perfil de prueba', description: 'Descripción persistida', permissionIds },
          7
        );
        createdIds.push(profile.id);
        const [rows] = await pool.execute(
          'SELECT name, description, created_by FROM profiles WHERE id = ?',
          [profile.id]
        );
        ...
        const [links] = await pool.execute(
          'SELECT permission_id FROM profile_permissions WHERE profile_id = ?',
          [profile.id]
        );
        assert.deepEqual(
          links.map((link) => link.permission_id),
      ...
      await assert.rejects(
        () => useCases.create({ name: 'Inválido', permissionIds: [permissionId, missingId] }),
        {
